...
# Bloqueo total (own-signal completamente ocluido) da SINR=0 lineal -> -inf dB.
# Para poder graficarlo, esos puntos se "clampean" a un piso visual (finito_min - 5 dB)
# y se marcan con un simbolo distinto; el valor real (-inf, outage) queda intacto en el CSV/JSON.
all_finite = [r["sinr_db"][str(rx)] for r in rows for rx in RX_IDXS
              if r["sinr_db"][str(rx)] != float("-inf")] + [gamma_th2_dB]
floor_dB = min(all_finite) - 5.0
hay_bloqueo_total = any(r["sinr_db"][str(rx)] == float("-inf") for r in rows for rx in RX_IDXS)

# ---------- Figura 1: SINR vs FOV ----------
fig, ax = plt.subplots(figsize=(9, 5), dpi=200)
for rx in RX_IDXS:
    y_raw = [r["sinr_db"][str(rx)] for r in rows]
    ...
    if bloqueados:
        ax.scatter(bloqueados, [floor_dB] * len(bloqueados), marker="x", s=90,
                   color=COLORS[rx], zorder=5, linewidths=2.5)
...
ax.set_title("SINR vs FOV — Escenario bloqueo persona, pitch 30°")
ax.set_xticks(fovs)
ax.set_ylim(floor_dB - 1.0, max(all_finite) + 1.0)
if hay_bloqueo_total:
    fig.text(0.01, -0.005, "× = bloqueo total de señal propia (SINR=0 lineal, -∞ dB), "
             "graficado en el piso del eje solo para visualización", fontsize=7.5, color="#898781")
...
ax.set_ylabel("Pout (%)")
ax.set_ylim(0, 100)
ax.set_title("Probabilidad de Outage vs FOV — Escenario bloqueo persona, pitch 30°")
...
def fmt_sinr(v):
    return "-∞ (bloqueo)" if v == float("-inf") else f"{v:.2f}"

col_labels = ["FOV (°)"] + [f"Asiento {seat_label_map[rx]}\n(dB)" for rx in RX_IDXS] + ["Promedio\n(dB)", "Mínimo\n(dB)"]
cell_text = []
for r in rows:
    fila = [f"{int(r['fov_deg'])}"] + [fmt_sinr(r["sinr_db"][str(rx)]) for rx in RX_IDXS] + \
           [fmt_sinr(r["sinr_prom_dB"]), fmt_sinr(r["sinr_min_dB"])]
